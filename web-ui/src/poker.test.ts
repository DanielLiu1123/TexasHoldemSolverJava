...
import { cellLabel, comboToCell, parseRange, serializeRange } from "./poker";
...
describe("comboToCell", () => {
  it("maps combos to grid cells", () => {
    expect(comboToCell("AhKh")).toBe("AKs");
    expect(comboToCell("AhKs")).toBe("AKo");
    expect(comboToCell("KsAh")).toBe("AKo");
    expect(comboToCell("7c7d")).toBe("77");
    expect(comboToCell("bogus")).toBeNull();
  });
});
