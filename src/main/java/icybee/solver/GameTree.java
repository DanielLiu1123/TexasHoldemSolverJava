...
public class GameTree {
    ...
    void buildChance(ChanceNode root, Rule rule) {
        ...
        for (Card _ : this.deck.getCards()) {
            ...
            if (rule.oop_commit == rule.ip_commit && rule.oop_commit == rule.stack) {
                ...
                one_node = new ActionNode(List.of(), List.of(), 1, this.intToGameRound(rule.current_round), (double) rule.get_pot(), root);
            }
            childrens.add(one_node);
            assert (nextrule.current_round <= 4);
    ...
    void buildAction(ActionNode root, Rule rule, String last_action, int check_times, int raise_times) {
        ...
        String[] possible_actions = switch (last_action) {
        ...
        for (String action : possible_actions) {
            switch (action) {
                case "check" -> {
                    ...
                    Rule nextrule;
                    if ((Objects.equals(last_action, "call") && root.getParent() != null && root.getParent().getParent() == null) || check_times >= 1) {
                        ...
                        nextrule = new Rule(rule);
                        nextnode = new ActionNode(List.of(), List.of(), nextplayer, this.intToGameRound(rule.current_round), (double) rule.get_pot(), root);
                    } else {
                        nextrule = new Rule(rule);
                        nextnode = new ActionNode(List.of(), List.of(), nextplayer, this.intToGameRound(rule.current_round), (double) rule.get_pot(), root);
                    }
                ...
                case "bet" -> {
                    ...
                    // if it's a donk bet
                    if (root.getPlayer() == 1 && root.getParent() != null && root.getParent() instanceof ChanceNode chanceNodeBeforeThis) {
                    ...
                    for (Double one_betting_size : bet_sizes) {
                        ...
                        GameTreeNode nextnode = new ActionNode(List.of(), List.of(), nextplayer, this.intToGameRound(rule.current_round), (double) rule.get_pot(), root);
        ...
        switch (node) {
            case ActionNode actionNode -> {
                ...
                for (int i = 0; i < childrens.size(); i++) {
                    ...
                    System.out.printf(
                            "%sp%s: %s%n", prefix, actionNode.getPlayer(), one_action.toString()
                    );
            ...
            case ShowdownNode showdown_node -> {
                ...
                prefix.repeat("\t", Math.max(0, depth));
                System.out.printf(
                        "%s SHOWDOWN pot %f %n", prefix.toString(), showdown_node.getPot()
                );
            ...
            case TerminalNode terminal_node -> {
                ...
                prefix.repeat("\t", Math.max(0, depth));
                System.out.printf(
                        "%s TERMINAL pot %f %n", prefix, terminal_node.getPot()
                );
