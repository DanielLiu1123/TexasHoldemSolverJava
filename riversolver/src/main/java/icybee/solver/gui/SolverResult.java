...
public class SolverResult {
    ...
    static class NodeDesc {
        ...
        NodeDesc(GameTreeNode node, @Nullable String last_action, int action_ind) {
            this.node = node;
        ...
        public Color getColor() {
            if (this.last_action == null) {
                ...
                if (action.getAction() == GameTreeNode.PokerActions.CALL
                        || action.getAction() == GameTreeNode.PokerActions.CHECK) {
                    return Color.GREEN;
                } else if (action.getAction() == GameTreeNode.PokerActions.BET
                        || action.getAction() == GameTreeNode.PokerActions.RAISE) {
                    return Color.RED;
                } else {
                    return Color.GRAY;
                }
    ...
    SolverResult(GameTree game_tree, GameTreeNode root, String boardstr) {
        ...
        strategy_table.addComponentListener(new ComponentAdapter() {
            ...
            public void componentResized(ComponentEvent e) {
                ...
                Dimension v = strategy_viewer_panel.getViewportBorderBounds().getSize();
                if (v.height > p.height) {
                    int available = v.height - strategy_table.getRowCount() * strategy_table.getRowMargin();
        ...
        detail_table.addComponentListener(new ComponentAdapter() {
            ...
            public void componentResized(ComponentEvent e) {
                ...
                if (v.height > p.height) {
                    int available = v.height - detail_table.getRowCount() * detail_table.getRowMargin();
    ...
    void update_global_strategy(NodeDesc desc) {
        ...
        if (global_node_desc == null || global_node_desc.node.getType() != GameTreeNode.GameTreeNodeType.ACTION) {
            return;
        }
        actionNode = (ActionNode) global_node_desc.node;
        DiscountedCfrTrainable dct = (DiscountedCfrTrainable) Objects.requireNonNull(actionNode.getTrainable());
        int player = actionNode.getPlayer();
        float[] reach_probs = Objects.requireNonNull(dct.getReach_probs())[player];
        float[] evs = Objects.requireNonNull(dct.getEvs());
        ...
        for (int i = 0; i < global_strategy.length; i++) {
            ...
            actions[i] = String.format(
                    "<html><h4>%s</h4><p style=\"font-size:7px\"> %.1f combos<br>%.1f %s</p></html>",
                    actionNode.getActions().get(i).toString(), combos[i], global_strategy[i] * 100, "%");
        }
        ...
        GlobalStrategyColorTableCellRenderer gcr =
                new GlobalStrategyColorTableCellRenderer(actionNode.getActions(), global_strategy, combos);
        global_info.setDefaultRenderer(Object.class, gcr);
        ComponentListener[] listeners = global_info.getComponentListeners();
        for (ComponentListener cl : listeners) global_info.removeComponentListener(cl);
        ...
        String board_str_toshow = this.boardstr
                .replace("c", "<font color=\"black\">♣</font>")
        ...
        String info_toshow =
                String.format("<html>board: %s<br><h3>%s decision node</h3></html>", board_str_toshow, player_toshow);
    ...
    void construct_inital_table() {
        if (this.game_tree.getDeck().getCards().size() == 52) {
            ...
            throw new RuntimeException(String.format(
                    "deck size %d unknown", this.game_tree.getDeck().getCards().size()));
        }
    ...
    void reGenerateTree(GameTreeNode node, DefaultMutableTreeNode parent, GameTreeNode.GameRound stop_round) {
        ...
        if (node instanceof ActionNode actionNode) {
            ...
            for (int i = 0; i < childs.size(); i++) {
                ...
                privateCards[actionNode.getPlayer()] =
                        ((DiscountedCfrTrainable) Objects.requireNonNull(actionNode.getTrainable())).getPrivateCards();
    ...
    class EachCellRenderer extends DefaultTableCellRenderer {
        ...
        public EachCellRenderer(int row, int colunm, NodeDesc desc, boolean selected) {
            ...
            float[][] reach_prob = Objects.requireNonNull(trainable.getReach_probs());
            List<GameActions> actions = actionNode.getActions();
            ...
            for (int i = 0; i < cards.length; i++) {
                ...
                for (int j = 0; j < actions.size(); j++) {
                    ...
                    node_strategy[j] =
                            node_strategy[j] * (num_cases - 1) / num_cases + strategy[strategy_index] / num_cases;
                }
            }
        ...
        public void paintStrategy(Graphics g) {
            if (node_strategy == null || actions == null || sum(node_strategy) == 0) {
                super.paintComponent(g);
                if (this.selected) paintBlackSide(g);
                return;
            }
    ...
    void setDetailStrategyInfo(int row, int colunm, NodeDesc desc) {
        ...
        float[] strategy = trainable.getAverageStrategy();
        float[] evs = Objects.requireNonNull(trainable.getEvs());
        List<GameActions> actions = actionNode.getActions();
        PrivateCards[] cards = trainable.getPrivateCards();
        float[][] reach_probs = Objects.requireNonNull(trainable.getReach_probs());

        if (showType == ShowType.STRATEGY) {
            List<DetailStrategyInfo> infos = new ArrayList<DetailStrategyInfo>();
            float norm_ev = sum(reach_probs[player]);
            ...
            for (int i = 0; i < cards.length; i++) {
                ...
                for (int j = 0; j < actions.size(); j++) {
                    int strategy_index = j * cards.length + i;
                    one_strategy[j] = strategy[strategy_index];
                    strategy_str += String.format(
                            "<p style=\"font-size:7px\">%s : %.1f %s </p>",
                            actions.get(j).toString(), one_strategy[j] * 100, "%");
                }
                String card_infos = String.format(
                        "<html><h2 style=\"background-color:rgb(255, 255, 255);\"> %s </h2><br><h3>EV: %.2f</h3>%s</html>",
                        one_private_card.toFormatString(), evs[i] / norm_ev, strategy_str);
                ;
            ...
            int line_num = 3;
            String[][] detail_grid_names = new String[line_num][columnName.length];
            float[][][] reach_prob3d = new float[2][3][columnName.length];
            for (int one_player = 0; one_player < 2; one_player++) {
                PrivateCards[] player_cards = privateCards[one_player];
                assert (reach_probs[one_player].length == player_cards.length);
                int num = 0;
                for (int i = 0; i < player_cards.length; i++) {
                    PrivateCards one_private_card = player_cards[i];
                    if (!name.equals(one_private_card.summary())) continue;
                    reach_prob3d[one_player][num / columnName.length][num % columnName.length] =
                            reach_probs[one_player][i];
    ...
    class StragetyDetailCellRenderer extends DefaultTableCellRenderer {
        ...
        public StragetyDetailCellRenderer(
            ...
            if (strategy2d == null
                    || !(row < strategy2d.length || colunm < strategy2d[0].length)
                    || strategy2d[row][colunm] == null) return;
    ...
    class StrategyDetailColorTableCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(
            ...
            StragetyDetailCellRenderer cell_renderer =
                    new StragetyDetailCellRenderer(row, column, this.strategy2d, this.reach_prob3d, isSelected);
            return cell_renderer.getTableCellRendererComponent(table, value, false, false, row, column);
        }
    }
    ...
    static class GlobalStrategyCellRenderer extends DefaultTableCellRenderer {
        ...
        public GlobalStrategyCellRenderer(
                int row, int colunm, boolean selected, GameActions action, float strategy, float combos) {
            this.row = row;
            this.colunm = colunm;
            this.selected = selected;
            this.action = action;
    ...
    static class GlobalStrategyColorTableCellRenderer extends DefaultTableCellRenderer {
        ...
        public GlobalStrategyColorTableCellRenderer(
                List<GameActions> actions, float[] global_strategy, float[] combos) {
            this.actions = actions;
