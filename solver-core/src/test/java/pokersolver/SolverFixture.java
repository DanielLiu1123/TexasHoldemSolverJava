...
final class SolverFixture {
    ...
    static final String RIVER_TREE = "gametree/river-tree.json";

    static final String TURN_TREE = "gametree/turn-tree.json";
    ...
    static float solveAndMeasure(SolverConfig.Builder builder, boolean parallel) throws Exception {
        ...
        SolverConfig config = builder.progressListener(
                ...
                .build();
        Solver solver = parallel ? new ParallelCfrSolver(config, 4, 1.0, 0.0, 1, 0) : new SequentialCfrSolver(config);
        solver.train();
        return last[0];
    }
}
